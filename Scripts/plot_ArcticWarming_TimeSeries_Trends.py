...
for i in range(len(mean)):
    ...
    if i > 0:
        plt.text(xx[i],mean[i]-std_err[i]-0.1,r'\textbf{%s}' % np.round(corrs[i-1],2),
             color='k',fontsize=9,ha='center',va='center',clip_on=False)
...
plt.savefig(directoryfigure + 'Arctic_75N_Trends_%s_%s-%s_%s.png' % (varnames[0],
